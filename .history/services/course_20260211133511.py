...
class CourseService:
    ...
    def update_course(course_id: int, course_update: CourseBase):
        ...
        # 3. The "Self-Collision" Check (Unique Code Logic)
        # We only check for duplicates if the user is actually CHANGING the code.
        if course_update.code != current_course.code:
            for id, course in courses_db.items():
                # Check if the code exists AND it belongs to a DIFFERENT course ID
        ...
        # 4. Apply the updates
        # We update the existing object in memory
        current_course.title = course_update.title
        current_course.code = course_update.code
        
        # In a real DB we would call .save() or .commit(), 
        # but in-memory, modifying the object updates the dictionary reference automatically.
