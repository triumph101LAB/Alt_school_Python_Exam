...
def deregitser_course(EnrollId:int, student: User = Depends(is_student_user)):
    return EnrollmentService.deregister(EnrollId,student)
...
def force_deregitser_course(enrollment_id:int, admin: User = Depends(is_admin_user)):
    return EnrollmentService.deregister(EnrollId,admin)