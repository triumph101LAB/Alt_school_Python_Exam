...
def deregitser_course(enrolltId:int, student: User = Depends(is_student_user)):
...
def get_course_enrollment(courseId:int, admin :User = Depends(is_admin_user)):
    return EnrollmentService.get_course_enrollment()

@enrollment_router.delete("force/{EnrollId}")
def deregitser_course(enrollmentId:int, admin: User = Depends(is_admin_user)):
    return EnrollmentService.deregister(enrollmentId)