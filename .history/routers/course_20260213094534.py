...
from services.deps import is_adm
course_router = APIRouter()

@course_router.post("/", status_code=status.HTTP_201_CREATED)
def create_course(course:CourseBase, admin: int = Depends(is_admint_user)):
...
@course_router.patch("/update_course")
def update_course(course_id:int, course:CourseBase,admin: User = Depends):
